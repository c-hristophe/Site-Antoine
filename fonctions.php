	...
	if(isset($_POST['envoyer'])){//si le bouton envoyer (name="envoyer") est cliqué, on traite le fomulaire
		...
		if($msg_erreur!=''){
			...
			if(mysqli_query($mysqli,"INSERT INTO commentaires SET 
				...
				echo "<p>Une erreur s'est produite, merci de réessayer ou contactez cr-éation.com si le problème persiste.</p>".mysqli_error($mysqli);
			}
		}
	}
	if($AfficherForm==1){
		?>
		<div style="text-align:center;margin-top:10px;margin-bottom:100px; background-image: url(img/cahier.jpg); background-repeat: no-repeat;background-position: center;">
		<br/><br/><br/><br/><form action="<?php echo $Page; ?>" method="post">
		<br/><br/>Pseudo
			<br/>
			...
			Message
			<br/>
			<textarea name="commentaire" rows="10" cols="30" required="required"><?php echo raf("commentaire"); ?></textarea>
			<!--
			...
			-->
			<br/><br/><br/>
			<input type="submit" style="width:200px; font-size:25px; border-radius:30px" name="envoyer" value="Poster"><br/><br/><br/><br/><br/><br/><br/>
		</form>
	</div>
		<?php
	}
...
function afficherCommentaires($IdArticle=0){
	...
	$req = mysqli_query($mysqli,"SELECT * FROM commentaires WHERE id_article ".($id!=0?"=$id": "IS NULL")." ORDER BY id DESC");
	if(mysqli_num_rows($req)==0) {
		echo "<p>Aucun commentaire pour le moment.</p>";
	} else {
		while($infos = mysqli_fetch_assoc($req)) {
		?>
		<div style="text-align:center;margin-bottom:30px;border-bottom:1px solid black">
			<p style="text-decoration:underline"><b><?php echo $infos['pseudo']; ?> le <?php echo date("d-m-Y",$infos['quand'])." à ".date("H:i",$infos['quand']); ?></b></p>
			<p style="text-align:center;width:40%;text-align:center; font-size:20px;font-family: 'Caveat', cursive; margin-left:30%"><?php echo $infos['commentaire']; ?></p>
		</div>
		<?php
		}
	}
